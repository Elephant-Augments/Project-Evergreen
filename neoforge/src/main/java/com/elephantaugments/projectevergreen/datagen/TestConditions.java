...
import com.elephantaugments.projectevergreen.input.Blacklist;
import com.elephantaugments.projectevergreen.input.Constants;
import com.elephantaugments.projectevergreen.input.DefaultStructureFixes;
...
public class TestConditions {
    ...
    public static void registerConditions() {
        ...
        Patched.registerSimpleTestCondition(ResourceLocation.fromNamespaceAndPath(ProjectEvergreen.MODID, Blacklist.IGNORE_BIOME_RADIUS), TestConditions::biomeRadiusIgnored);
        Patched.registerSimpleTestCondition(ResourceLocation.fromNamespaceAndPath(ProjectEvergreen.MODID, Blacklist.IGNORE_BIOME_REDISTRIBUTION), TestConditions::biomeRedistributionIgnored);
    ...
    private static boolean biomeRedistributionIgnored(JsonElement value) {
		final ResourceLocation id = PatchUtil.assertIsResourceLocation(Blacklist.IGNORE_BIOME_REDISTRIBUTION, "value", value);
        return isBiomeRedistributionIgnored(id.toString());
	}

    private static boolean biomeRadiusIgnored(JsonElement value) {
		final ResourceLocation id = PatchUtil.assertIsResourceLocation(Blacklist.IGNORE_BIOME_RADIUS, "value", value);
    ...
    public static boolean isBiomeLoaded(String id) {
        ...
        final String fullPath = "worldgen/biome/" + Constants.getPath(id);
        final String namespace = Constants.getNamespace(id);
        
        return Constants.loadedBiomes.entries().stream()
            .filter(e -> namespace.equals(e.getKey()))
            .filter(e -> fullPath.equals(e.getValue()))
            .count() > 0;
    }
    ...
    public static boolean isStructureLoaded(String id) {
        ...
        final String fullPath = "worldgen/structure/" + Constants.getPath(id);
        final String namespace = Constants.getNamespace(id);
        
        return Constants.loadedStructures.entries().stream()
            .filter(e -> namespace.equals(e.getKey()))
            .filter(e -> fullPath.equals(e.getValue()))
            .count() > 0;
    }
    ...
    public static boolean isIgnored(String id) {
        return ProjectEvergreenConfig.structuresByFix.entries().stream()
            ...
            .anyMatch(e -> e.equals(Blacklist.IGNORE_MOD) || e.equals(Blacklist.IGNORE_STRUCTURE));
    }
    ...
    public static boolean isStructureTypeIgnored(String id) {
        return ProjectEvergreenConfig.structuresByFix.entries().stream()
            ...
            .map(Map.Entry::getKey)
            .anyMatch(e -> e.equals(Blacklist.IGNORE_STRUCTURE_TYPE));
    }
    ...
    public static boolean isBiomeRedistributionIgnored(String id) {
        return ProjectEvergreenConfig.structuresByFix.entries().stream()
            .filter(e -> id.equals(e.getValue()))
            .map(Map.Entry::getKey)
            .anyMatch(e -> e.equals(Blacklist.IGNORE_BIOME_REDISTRIBUTION));
    }

    public static boolean isBiasIgnored(String id) {
        return (ALL_IGNORED_BIAS_STRUCTURES.indexOf(id) > 0) || (Blacklist.ignoreBiomeRadius.indexOf(id) > 0);
    }
    ...
    public static boolean isTerrainAdaptationIgnored(String id) {
        return ProjectEvergreenConfig.structuresByFix.entries().stream()
            .filter(e -> id.equals(e.getValue()))
            .map(Map.Entry::getKey)
            .anyMatch(e -> e.equals(Blacklist.IGNORE_TERRAIN_ADAPTATION));
    }
    ...
    public static boolean isFlatnessCheckIgnored(String id) {
        return ProjectEvergreenConfig.structuresByFix.entries().stream()
            .filter(e -> id.equals(e.getValue()))
            .map(Map.Entry::getKey)
            .anyMatch(e -> e.equals(Blacklist.IGNORE_FLATNESS_CHECK));
    }
