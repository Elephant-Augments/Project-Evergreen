...
public class DataSources {
    ...
    public static void registerDataSources() {
        ...
        SingleDataSource source = (from, value) -> {
            return switch (value.getAsString()) {
                ...
                //case Constants.PERFORMANCE_CONFIG_VALUE -> DataSources.getPerformanceMode();
                //case Constants.STRUCTURE_RARITY_REDISTRIBUTION -> DataSources.getRarityRedistribution();
                ...
                case Constants.LOST_CITIES_BIOME -> DataSources.getLostCitiesBiome();
                case Constants.LOST_CITIES_LIQUID -> DataSources.getLostCitiesLiquid();
    ...
    public static JsonElement getLostCitiesBiome() {
        JsonElement biome = TestConditions.isBiomeLoaded(ProjectEvergreenConfig.lostCitiesBiome) ?
            ProjectEvergreen.GSON.toJsonTree(ProjectEvergreenConfig.lostCitiesBiome) :
            ProjectEvergreen.GSON.toJsonTree("minecraft:forest");
        return biome;
    }

    public static JsonElement getLostCitiesLiquid() {
        final ResourceLocation rl = ResourceLocation.tryParse(ProjectEvergreenConfig.lostCitiesLiquid);
        JsonElement liquid = BuiltInRegistries.BLOCK.containsKey(rl) ?
            ProjectEvergreen.GSON.toJsonTree(ProjectEvergreenConfig.lostCitiesLiquid) :
            ProjectEvergreen.GSON.toJsonTree("minecraft:water");
        return liquid;
    }
    ...
    private static JsonElement getSpreadWithOffset(Integer spread, Double rarity, String id) {
        return  (Patched.platform().isModLoaded("integrated_api") &&
                (!ProjectEvergreenConfig.performanceFriendlyMode || TestConditions.hasPopulationBias_StructureSet(id))) ? 
            ProjectEvergreen.GSON.toJsonTree(Math.ceil(spread * Constants.INTEGRATED_API_SPREAD_OFFSET * rarity)) :
    ...
    private static JsonElement buildTagSet(List<JsonElement> set) {
        List<String> jsonSet = set.stream()
            ...
            .filter(e -> TestConditions.isBiomeLoaded(e))
    ...
    private static JsonElement buildStructureSet(List<? extends String> set) {
        List<JsonObject> jsonSet = set.stream()
            .filter(e -> TestConditions.isStructureLoaded(e) && !Blacklist.structureIDs.contains(e))
    ...
    private static JsonObject buildStructureEntry(String id) {
        ...
        } else if (TestConditions.isFlatStructure(id) && TestConditions.isMassiveStructure(id)) {
