package com.elephantaugments.projectevergreen.common.data.defaults;

import com.elephantaugments.projectevergreen.common.api.PatchableBiome;
import com.elephantaugments.projectevergreen.common.data.patchable.PatchableBiomes;
import com.google.common.collect.ArrayListMultimap;
import com.google.common.collect.ImmutableList;

import java.util.*;

public class DefaultBiomeTags {

    public static final String DESERT_DUNES_ALIVE = "#project_evergreen:desert_dunes_alive";
    public static final String DESERT_DUNES_BARREN = "#project_evergreen:desert_dunes_barren";
    public static final String DESERT_RED_ALIVE = "#project_evergreen:desert_red_alive";
    public static final String DESERT_RED_BARREN = "#project_evergreen:desert_red_barren";
    public static final String FOREST_DENSE_CONIFEROUS_SNOW = "#project_evergreen:forest_dense_coniferous_snow";
    public static final String FOREST_DENSE_CONIFEROUS = "#project_evergreen:forest_dense_coniferous";
    public static final String FOREST_DENSE_DECIDUOUS = "#project_evergreen:forest_dense_deciduous";
    public static final String FOREST_DENSE_TROPICAL = "#project_evergreen:forest_dense_tropical";
    public static final String FOREST_SPARSE_CONIFEROUS_SNOW = "#project_evergreen:forest_sparse_coniferous_snow";
    public static final String FOREST_SPARSE_CONIFEROUS = "#project_evergreen:forest_sparse_coniferous";
    public static final String FOREST_SPARSE_DECIDUOUS = "#project_evergreen:forest_sparse_deciduous";
    public static final String FOREST_SPARSE_TROPICAL = "#project_evergreen:forest_sparse_tropical";
    public static final String MOUNTAINS_ALIVE = "#project_evergreen:mountains_alive";
    public static final String MOUNTAINS_BARREN = "#project_evergreen:mountains_barren";
    public static final String MOUNTAINS_COLD = "#project_evergreen:mountains_cold";
    public static final String MOUNTAINS_HOT = "#project_evergreen:mountains_hot";
    public static final String OCEAN_DEEP_FROZEN = "#project_evergreen:ocean_deep_frozen";
    public static final String OCEAN_DEEP_TEMPERATE = "#project_evergreen:ocean_deep_temperate";
    public static final String OCEAN_DEEP_WARM = "#project_evergreen:ocean_deep_warm";
    public static final String OCEAN_SHALLOW_FROZEN = "#project_evergreen:ocean_shallow_frozen";
    public static final String OCEAN_SHALLOW_TEMPERATE = "#project_evergreen:ocean_shallow_temperate";
    public static final String OCEAN_SHALLOW_WARM = "#project_evergreen:ocean_shallow_warm";
    public static final String COASTAL_FROZEN = "#project_evergreen:coastal_frozen";
    public static final String COASTAL_TEMPERATE = "#project_evergreen:coastal_temperate";
    public static final String COASTAL_WARM = "#project_evergreen:coastal_warm";
    public static final String COASTAL_ALIVE = "#project_evergreen:coastal_alive";
    public static final String COASTAL_BARREN = "#project_evergreen:coastal_barren";
    public static final String RIVER_FROZEN = "#project_evergreen:river_frozen";
    public static final String RIVER_TEMPERATE = "#project_evergreen:river_temperate";
    public static final String RIVER_WARM = "#project_evergreen:river_warm";
    public static final String PLAINS_GRASSY_ARID = "#project_evergreen:plains_grassy_arid";
    public static final String PLAINS_GRASSY_SNOW = "#project_evergreen:plains_grassy_snow";
    public static final String PLAINS_GRASSY_TEMPERATE = "#project_evergreen:plains_grassy_temperate";
    public static final String PLAINS_GRASSY_TROPICAL= "#project_evergreen:plains_grassy_tropical";
    public static final String PLAINS_SHRUBBY_ARID = "#project_evergreen:plains_shrubby_arid";
    public static final String PLAINS_SHRUBBY_SNOW = "#project_evergreen:plains_shrubby_snow";
    public static final String PLAINS_SHRUBBY_TEMPERATE = "#project_evergreen:plains_shrubby_temperate";
    public static final String PLAINS_SHRUBBY_TROPICAL = "#project_evergreen:plains_shrubby_tropical";
    public static final String SPECIAL_AUTUMNAL_FIELDS = "#project_evergreen:special_autumnal_fields";
    public static final String SPECIAL_AUTUMNAL_FOREST = "#project_evergreen:special_autumnal_forest";
    public static final String SPECIAL_CRAGGY_SNOW = "#project_evergreen:special_craggy_snow";
    public static final String SPECIAL_CRAGGY_TEMPERATE = "#project_evergreen:special_craggy_temperate";
    public static final String SPECIAL_CRAGGY_WARM = "#project_evergreen:special_craggy_warm";
    public static final String SPECIAL_FLOWERY_FIELDS = "#project_evergreen:special_flowery_fields";
    public static final String SPECIAL_FLOWERY_FOREST = "#project_evergreen:special_flowery_forest";
    public static final String SPECIAL_MEDITERRANEAN_INVITING = "#project_evergreen:special_mediterranean_inviting";
    public static final String SPECIAL_MEDITERRANEAN_UNINVITING = "#project_evergreen:special_mediterranean_uninviting";
    public static final String SPECIAL_ORIENTAL_INVITING = "#project_evergreen:special_oriental_inviting";
    public static final String SPECIAL_ORIENTAL_UNINVITING = "#project_evergreen:special_oriental_uninviting";
    public static final String SPECIAL_ICY = "#project_evergreen:special_icy";
    public static final String SPECIAL_MAGICAL = "#project_evergreen:special_magical";
    public static final String SPECIAL_RUINED = "#project_evergreen:special_ruined";
    public static final String SPECIAL_SHROOMY = "#project_evergreen:special_shroomy";
    public static final String SPECIAL_SPOOKY = "#project_evergreen:special_spooky";
    public static final String SPECIAL_SWAMPY_SNOW = "#project_evergreen:special_swampy_snow";
    public static final String SPECIAL_SWAMPY_TEMPERATE = "#project_evergreen:special_swampy_temperate";
    public static final String SPECIAL_SWAMPY_WARM = "#project_evergreen:special_swampy_warm";

    public static LinkedHashMap<String, PatchableBiome> buildMapEntries(PatchableBiomes biomeData) {
        LinkedHashMap<String, PatchableBiome> biomes = new LinkedHashMap<>();
        ArrayListMultimap<String, PatchableBiome> biomesByTag = mapBiomesToTags(biomeData);
        biomeData.Data.forEach((id, biome) -> {
            biome.setBiomeTags(getBiomeTags(biomesByTag, id));
            biomes.put(id, biome);
        });
        return biomes;
    }

    public static LinkedHashMap<String, PatchableBiome> buildMapEntries(PatchableBiomes biomeData, ArrayListMultimap<String, PatchableBiome> biomesByTag) {
        LinkedHashMap<String, PatchableBiome> biomes = new LinkedHashMap<>();
        biomeData.Data.forEach((id, biome) -> {
            biome.setBiomeTags(getBiomeTags(biomesByTag, id));
            biomes.put(id, biome);
        });
        return biomes;
    }

    private static List<String> getBiomeTags(ArrayListMultimap<String, PatchableBiome> biomeIDsByTag, String id) {
        List<String> list = new ArrayList<>();
        Set<String> uniqueValues = new HashSet<>();
        for (Map.Entry<String, PatchableBiome> e : biomeIDsByTag.entries()) {
            if (e.getValue() != null && e.getValue().id.equals(id)) {
                String key = e.getKey();
                if (uniqueValues.add(key)) {
                    list.add(key);
                }
            }
        }
        return list;
    }

    public static ArrayListMultimap<String, PatchableBiome> mapBiomesToTags(PatchableBiomes biomeData) {
		ArrayListMultimap<String, PatchableBiome> biomesByTag = ArrayListMultimap.create();
        desertDunesAlive.forEach(s -> biomesByTag.put(DESERT_DUNES_ALIVE, biomeData.Data.get(s)));
        desertDunesBarren.forEach(s -> biomesByTag.put(DESERT_DUNES_BARREN, biomeData.Data.get(s)));
        desertRedAlive.forEach(s -> biomesByTag.put(DESERT_RED_ALIVE, biomeData.Data.get(s)));
        desertRedBarren.forEach(s -> biomesByTag.put(DESERT_RED_BARREN, biomeData.Data.get(s)));
        forestDenseConiferousSnow.forEach(s -> biomesByTag.put(FOREST_DENSE_CONIFEROUS_SNOW, biomeData.Data.get(s)));
        forestDenseConiferous.forEach(s -> biomesByTag.put(FOREST_DENSE_CONIFEROUS, biomeData.Data.get(s)));
        forestDenseDeciduous.forEach(s -> biomesByTag.put(FOREST_DENSE_DECIDUOUS, biomeData.Data.get(s)));
        forestDenseTropical.forEach(s -> biomesByTag.put(FOREST_DENSE_TROPICAL, biomeData.Data.get(s)));
        forestSparseConiferousSnow.forEach(s -> biomesByTag.put(FOREST_SPARSE_CONIFEROUS_SNOW, biomeData.Data.get(s)));
        forestSparseConiferous.forEach(s -> biomesByTag.put(FOREST_SPARSE_CONIFEROUS, biomeData.Data.get(s)));
        forestSparseDeciduous.forEach(s -> biomesByTag.put(FOREST_SPARSE_DECIDUOUS, biomeData.Data.get(s)));
        forestSparseTropical.forEach(s -> biomesByTag.put(FOREST_SPARSE_TROPICAL, biomeData.Data.get(s)));
        mountainsAlive.forEach(s -> biomesByTag.put(MOUNTAINS_ALIVE, biomeData.Data.get(s)));
        mountainsBarren.forEach(s -> biomesByTag.put(MOUNTAINS_BARREN, biomeData.Data.get(s)));
        mountainsCold.forEach(s -> biomesByTag.put(MOUNTAINS_COLD, biomeData.Data.get(s)));
        mountainsHot.forEach(s -> biomesByTag.put(MOUNTAINS_HOT, biomeData.Data.get(s)));
        oceanDeepFrozen.forEach(s -> biomesByTag.put(OCEAN_DEEP_FROZEN, biomeData.Data.get(s)));
        oceanDeepTemperate.forEach(s -> biomesByTag.put(OCEAN_DEEP_TEMPERATE, biomeData.Data.get(s)));
        oceanDeepWarm.forEach(s -> biomesByTag.put(OCEAN_DEEP_WARM, biomeData.Data.get(s)));
        oceanShallowFrozen.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_FROZEN, biomeData.Data.get(s)));
        oceanShallowTemperate.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_TEMPERATE, biomeData.Data.get(s)));
        oceanShallowWarm.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_WARM, biomeData.Data.get(s)));
        coastalFrozen.forEach(s -> biomesByTag.put(COASTAL_FROZEN, biomeData.Data.get(s)));
        coastalTemperate.forEach(s -> biomesByTag.put(COASTAL_TEMPERATE, biomeData.Data.get(s)));
        coastalWarm.forEach(s -> biomesByTag.put(COASTAL_WARM, biomeData.Data.get(s)));
        coastalAlive.forEach(s -> biomesByTag.put(COASTAL_ALIVE, biomeData.Data.get(s)));
        coastalBarren.forEach(s -> biomesByTag.put(COASTAL_BARREN, biomeData.Data.get(s)));
        riverFrozen.forEach(s -> biomesByTag.put(RIVER_FROZEN, biomeData.Data.get(s)));
        riverTemperate.forEach(s -> biomesByTag.put(RIVER_TEMPERATE, biomeData.Data.get(s)));
        riverWarm.forEach(s -> biomesByTag.put(RIVER_WARM, biomeData.Data.get(s)));
        plainsGrassyArid.forEach(s -> biomesByTag.put(PLAINS_GRASSY_ARID, biomeData.Data.get(s)));
        plainsGrassySnow.forEach(s -> biomesByTag.put(PLAINS_GRASSY_SNOW, biomeData.Data.get(s)));
        plainsGrassyTemperate.forEach(s -> biomesByTag.put(PLAINS_GRASSY_TEMPERATE, biomeData.Data.get(s)));
        plainsGrassyTropical.forEach(s -> biomesByTag.put(PLAINS_GRASSY_TROPICAL, biomeData.Data.get(s)));
        plainsShrubbyArid.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_ARID, biomeData.Data.get(s)));
        plainsShrubbySnow.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_SNOW, biomeData.Data.get(s)));
        plainsShrubbyTemperate.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_TEMPERATE, biomeData.Data.get(s)));
        plainsShrubbyTropical.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_TROPICAL, biomeData.Data.get(s)));
        specialAutumnalFields.forEach(s -> biomesByTag.put(SPECIAL_AUTUMNAL_FIELDS, biomeData.Data.get(s)));
        specialAutumnalForest.forEach(s -> biomesByTag.put(SPECIAL_AUTUMNAL_FOREST, biomeData.Data.get(s)));
        specialCraggySnow.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_SNOW, biomeData.Data.get(s)));
        specialCraggyTemperate.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_TEMPERATE, biomeData.Data.get(s)));
        specialCraggyWarm.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_WARM, biomeData.Data.get(s)));
        specialFloweryFields.forEach(s -> biomesByTag.put(SPECIAL_FLOWERY_FIELDS, biomeData.Data.get(s)));
        specialFloweryForest.forEach(s -> biomesByTag.put(SPECIAL_FLOWERY_FOREST, biomeData.Data.get(s)));
        specialMediterraneanInviting.forEach(s -> biomesByTag.put(SPECIAL_MEDITERRANEAN_INVITING, biomeData.Data.get(s)));
        specialMediterraneanUninviting.forEach(s -> biomesByTag.put(SPECIAL_MEDITERRANEAN_UNINVITING, biomeData.Data.get(s)));
        specialOrientalInviting.forEach(s -> biomesByTag.put(SPECIAL_ORIENTAL_INVITING, biomeData.Data.get(s)));
        specialOrientalUninviting.forEach(s -> biomesByTag.put(SPECIAL_ORIENTAL_UNINVITING, biomeData.Data.get(s)));
        specialIcy.forEach(s -> biomesByTag.put(SPECIAL_ICY, biomeData.Data.get(s)));
        specialMagical.forEach(s -> biomesByTag.put(SPECIAL_MAGICAL, biomeData.Data.get(s)));
        specialRuined.forEach(s -> biomesByTag.put(SPECIAL_RUINED, biomeData.Data.get(s)));
        specialShroomy.forEach(s -> biomesByTag.put(SPECIAL_SHROOMY, biomeData.Data.get(s)));
        specialSpooky.forEach(s -> biomesByTag.put(SPECIAL_SPOOKY, biomeData.Data.get(s)));
        specialSwampySnow.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_SNOW, biomeData.Data.get(s)));
        specialSwampyTemperate.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_TEMPERATE, biomeData.Data.get(s)));
        specialSwampyWarm.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_WARM, biomeData.Data.get(s)));
        biomesByTag.values().removeIf(Objects::isNull);
        return biomesByTag;
    }

    public static final List<String> desertDunesAlive = ImmutableList.of(
        "minecraft:desert",
        "terralith:desert_oasis",
        "regions_unexplored:joshua_desert",
        "regions_unexplored:saguaro_desert",
        "dreamwoods:arid_plains",
        "wythers:cactus_desert",
        "wythers:danakil_desert",
        "wythers:desert_lakes",
        "wythers:guelta",
        "wythers:outback_desert",
		"wythers:wooded_desert"
    );

    
    public static final List<String> desertDunesBarren = ImmutableList.of(
        "minecraft:desert",
        "terralith:desert_spires",
        "terralith:white_mesa",
        "natures_spirit:scorched_dunes",
        "natures_spirit:drylands",
        "wythers:deep_desert",
        "wythers:desert_pinnacles"
    );
    
    public static final List<String> desertRedAlive = ImmutableList.of(
        "minecraft:wooded_badlands",
        "natures_spirit:woody_highlands",
        "terralith:red_oasis",
        "terralith:arid_highlands",
        "terralith:savanna_badlands",
        "terralith:ashen_savanna",
        "terralith:bryce_canyon",
        "natures_spirit:blooming_dunes",
        "natures_spirit:lively_dunes",
        "dreamwoods:grassy_badlands"
    );
    
    public static final List<String> desertRedBarren = ImmutableList.of(
        "minecraft:badlands",
        "minecraft:eroded_badlands",
        "terralith:desert_canyon",
        "terralith:sandstone_valley",
        "terralith:ancient_sands",
        "natures_spirit:stratified_desert",
        "wythers:ayers_rock",
        "wythers:badlands_canyon",
		"wythers:badlands_desert",
        "wythers:red_desert",
		"wythers:red_rock_canyon"
    );
    
    public static final List<String> forestDenseConiferousSnow = ImmutableList.of(
        "minecraft:snowy_taiga",
        "terralith:siberian_grove",
        "terralith:snowy_shield",
        "terralith:siberian_taiga",
        "terralith:snowy_maple_forest",
        "terralith:wintry_forest",
        "terralith:wintry_lowlands",
        "natures_spirit:snowy_fir_forest",
        "regions_unexplored:cold_boreal_taiga",
        "regions_unexplored:cold_deciduous_forest",
        "regions_unexplored:spires",
        "atmospheric:kousa_jungle",
        "environmental:snowy_old_growth_pine_barrens",
        "environmental:snowy_pine_barrens",
        "environmental:old_growth_pine_barrens",
        "environmental:pine_barrens",
        "environmental:pine_slopes",
        "dreamwoods:autumnal_hills",
        "dreamwoods:snowy_pinelands",
        "windswept:snowy_chestnut_forest",
        "windswept:snowy_pine_barrens",
        "wythers:deep_snowy_taiga",
        "wythers:snowy_thermal_taiga"
    );
    
    public static final List<String> forestDenseConiferous = ImmutableList.of(
        "minecraft:taiga",
        "minecraft:old_growth_pine_taiga",
        "minecraft:old_growth_spruce_taiga",
        "terralith:shield",
        "natures_spirit:boreal_taiga",
        "regions_unexplored:pine_taiga",
        "regions_unexplored:boreal_taiga",
        "regions_unexplored:golden_boreal_taiga",
        "regions_unexplored:maple_forest",
        "natures_spirit:fir_forest",
        "dreamwoods:seasonal_taiga",
        "dreamwoods:fir_forest",
        "dreamwoods:spruce_slopes",
        "windswept:pine_barrens",
        "wythers:ancient_taiga",
        "wythers:boreal_forest_red",
		"wythers:boreal_forest_yellow",
        "wythers:old_growth_taiga_crags",
        "wythers:taiga_crags",
        "wythers:larch_taiga",
        "wythers:pine_barrens",
        "wythers:thermal_taiga",
		"wythers:thermal_taiga_crags"
    );
    
    public static final List<String> forestDenseDeciduous = ImmutableList.of(
        "minecraft:forest",
        "terralith:birch_taiga",
        "terralith:lush_valley",
        "terralith:blooming_valley",
        "regions_unexplored:deciduous_forest",
        "natures_spirit:sugi_forest",
        "regions_unexplored:willow_forest",
        "regions_unexplored:redwoods",
        "natures_spirit:redwood_forest",
        "regions_unexplored:maple_forest",
        "regions_unexplored:silver_birch_forest",
        "natures_spirit:aspen_forest",
        "dreamwoods:redwood_forest",
        "dreamwoods:temperate_hemlock_rainforest",
        "dreamwoods:wet_larch_forest",
        "dreamwoods:golden_aspen_spinney",
        "dreamwoods:wooded_slopes",
        "dreamwoods:weeping_grove",
        "windswept:chestnut_forest",
        "wythers:ancient_copper_beech_forest",
		"wythers:ancient_emerald_beech_forest",
		"wythers:ancient_golden_beech_forest",
        "wythers:ancient_moss_forest",
        "wythers:forested_highlands",
        "wythers:cool_forest",
        "wythers:giant_sequoia_forest",
        "wythers:maple_mountains",
        "wythers:temperate_rainforest",
		"wythers:temperate_rainforest_crags",
        "wythers:warm_birch_forest"
    );
    
    public static final List<String> forestDenseTropical = ImmutableList.of(
        "minecraft:jungle",
        "terralith:tropical_jungle",
        "terralith:amethyst_rainforest",
        "regions_unexplored:rainforest",
        "regions_unexplored:tropics",
        "natures_spirit:tropical_woods",
        "regions_unexplored:eucalyptus_forest",
        "dreamwoods:flooded_rainforest",
        "dreamwoods:roofed_tropical_forest",
        "wythers:bamboo_jungle_canyon",
		"wythers:bamboo_jungle_highlands",
		"wythers:eucalyptus_jungle",
		"wythers:eucalyptus_jungle_canyon",
        "wythers:eucalyptus_deanei_forest",
        "wythers:eucalyptus_salubris_woodland",
		"wythers:eucalyptus_woodland",
        "wythers:flooded_jungle",
		"wythers:flooded_rainforest",
        "wythers:subtropical_forest",
        "wythers:highland_tropical_rainforest",
        "wythers:tropical_forest",
		"wythers:tropical_forest_canyon",
		"wythers:tropical_rainforest",
		"wythers:windswept_jungle"
    );
    
    public static final List<String> forestSparseConiferousSnow = ImmutableList.of(
        "minecraft:snowy_plains",
        "terralith:highlands",
        "terralith:shield",
        "terralith:alpine_highlands",
        "terralith:forested_highlands",
        "terralith:temperate_highlands",
        "regions_unexplored:icy_heights",
        "dreamwoods:fall_snowy_grove",
        "dreamwoods:snowy_forest"
    );
    
    public static final List<String> forestSparseConiferous = ImmutableList.of(
        "minecraft:meadow",
        "terralith:yosemite_lowlands",
        "terralith:shield_clearing",
        "terralith:alpine_highlands",
        "terralith:forested_highlands",
        "terralith:temperate_highlands",
        "regions_unexplored:towering_cliffs",
        "natures_spirit:coniferous_covert",
        "dreamwoods:taiga_shield",
        "dreamwoods:alpine_fields",
        "dreamwoods:spruce_lowlands",
        "wythers:birch_taiga"
    );
    
    public static final List<String> forestSparseDeciduous = ImmutableList.of(
        "minecraft:birch_forest",
        "terralith:temperate_highlands",
        "terralith:forested_highlands",
        "terralith:valley_clearing",
        "regions_unexplored:orchard",
        "regions_unexplored:temperate_grove",
        "regions_unexplored:sparse_redwoods",
        "regions_unexplored:prairie",
        "natures_spirit:cypress_fields",
        "natures_spirit:oak_savanna",
        "natures_spirit:floral_ridges",
        "dreamwoods:spinney",
        "dreamwoods:poplar_grove",
        "dreamwoods:poplar_slopes",
        "windswept:lavender_hills",
        "wythers:forest_edge",
		"wythers:cool_forest_edge"
    );
    
    public static final List<String> forestSparseTropical = ImmutableList.of(
        "minecraft:sparse_jungle",
        "terralith:rocky_jungle",
        "regions_unexplored:sparse_rainforest",
        "natures_spirit:sparse_tropical_woods",
        "wythers:sparse_bamboo_jungle",
		"wythers:sparse_eucalyptus_jungle",
		"wythers:sparse_eucalyptus_woodland",
		"wythers:subtropical_forest_edge",
        "wythers:tropical_island"
    );
    
    public static final List<String> mountainsAlive = ImmutableList.of(
        "minecraft:grove",
        "terralith:jungle_mountains",
        "regions_unexplored:arid_mountains",
        "terralith:mountain_steppe",
        "terralith:savanna_slopes",
        "terralith:alpine_grove",
        "regions_unexplored:pine_slopes",
        "dreamwoods:poplar_slopes",
        "wythers:maple_mountains"
    );
    
    public static final List<String> mountainsBarren = ImmutableList.of(
        "minecraft:jagged_peaks",
        "minecraft:stony_peaks",
        "terralith:scarlet_mountains",
        "terralith:painted_mountains",
        "terralith:volcanic_peaks",
        "terralith:volcanic_crater",
        "terralith:caldera",
        "terralith:emerald_peaks",
        "terralith:rocky_mountains",
        "natures_spirit:red_peaks",
        "natures_spirit:sleeted_slopes",
        "wythers:tibesti_mountains"
    );
    
    public static final List<String> mountainsCold = ImmutableList.of(
        "minecraft:snowy_slopes",
        "minecraft:frozen_peaks",
        "minecraft:jagged_peaks",
        "terralith:scarlet_mountains",
        "terralith:alpine_grove",
        "terralith:emerald_peaks",
        "terralith:rocky_mountains",
        "terralith:mountain_steppe",
        "terralith:glacial_chasm",
        "terralith:frozen_cliffs",
        "regions_unexplored:mountains",
        "regions_unexplored:pine_slopes",
        "regions_unexplored:icy_heights",
        "natures_spirit:red_peaks",
        "natures_spirit:snowcapped_red_peaks",
        "natures_spirit:sleeted_slopes",
        "wythers:maple_mountains",
        "wythers:snowy_peaks",
        "wythers:tibesti_mountains"
    );
    
    public static final List<String> mountainsHot = ImmutableList.of(
        "minecraft:stony_peaks",
        "terralith:painted_mountains",
        "terralith:volcanic_peaks",
        "terralith:volcanic_crater",
        "terralith:caldera",
        "terralith:savanna_slopes",
        "terralith:jungle_mountains",
        "regions_unexplored:arid_mountains",
        "natures_spirit:dusty_slopes",
        "dreamwoods:volcano_fields",
        "dreamwoods:volcano_slopes",
        "wythers:icy_volcano",
        "wythers:volcano",
        "wythers:tropical_volcano"
    );
    
    public static final List<String> oceanDeepFrozen = ImmutableList.of(
        "minecraft:deep_frozen_ocean",
        "wythers:deep_icy_ocean"
    );
    
    public static final List<String> oceanDeepTemperate = ImmutableList.of(
        "minecraft:deep_ocean",
        "minecraft:deep_cold_ocean",
        "regions_unexplored:hyacinth_deeps"
    );
    
    public static final List<String> oceanDeepWarm = ImmutableList.of(
        "minecraft:deep_lukewarm_ocean"
    );

    public static final List<String> oceanDeepRare = ImmutableList.of(
        "regions_unexplored:hyacinth_deeps",
        "jellyfishing:rock_bottom"
    );
    
    public static final List<String> oceanShallowFrozen = ImmutableList.of(
        "minecraft:frozen_ocean",
        "wythers:ice_cap",
		"wythers:icy_ocean"
    );
    
    public static final List<String> oceanShallowTemperate = ImmutableList.of(
        "minecraft:ocean",
        "minecraft:cold_ocean"
    );
    
    public static final List<String> oceanShallowWarm = ImmutableList.of(
        "minecraft:warm_ocean",
        "minecraft:lukewarm_ocean",
        "regions_unexplored:rocky_reef",
        "wythers:coastal_mangroves"
    );

    public static final List<String> oceanShallowRare = ImmutableList.of(
        "jellyfishing:jellyfish_fields"
    );
    
    public static final List<String> coastalFrozen = ImmutableList.of(
        "minecraft:snowy_beach",
        "dreamwoods:frozen_shore",
        "wythers:cold_stony_shore",
        "wythers:frigid_island",
		"wythers:frozen_island",
		"wythers:icy_shore"
    );
    
    public static final List<String> coastalTemperate = ImmutableList.of(
        "minecraft:beach",
        "minecraft:stony_shore",
        "terralith:gravel_beach",
        "regions_unexplored:gravel_beach",
        "dreamwoods:gravel_beach",
        "dreamwoods:white_shore",
        "wythers:sand_dunes",
        "wythers:calcite_coast",
        "wythers:gravelly_beach",
        "wythers:temperate_island"
    );
    
    public static final List<String> coastalWarm = ImmutableList.of(
        "dreamwoods:azure_cove",
        "natures_spirit:tropical_shores",
        "wythers:black_beach",
        "wythers:deepslate_shore",
        "wythers:desert_beach",
		"wythers:jungle_island",
        "wythers:tropical_beach"
    );
    
    public static final List<String> coastalAlive = ImmutableList.of(
        "minecraft:beach",
        "dreamwoods:azure_cove",
        "natures_spirit:tropical_shores",
		"wythers:jungle_island",
        "wythers:temperate_island",
        "wythers:tropical_beach"
    );
    
    public static final List<String> coastalBarren = ImmutableList.of(
        "minecraft:stony_shore",
        "terralith:gravel_beach",
        "dreamwoods:frozen_shore",
        "regions_unexplored:gravel_beach",
        "wythers:cold_stony_shore",
        "wythers:gravelly_beach"
    );
    
    public static final List<String> riverFrozen = ImmutableList.of(
        "minecraft:frozen_river",
        "wythers:icy_river"
    );
    
    public static final List<String> riverTemperate = ImmutableList.of(
        "minecraft:river",
        "regions_unexplored:cold_river",
        "regions_unexplored:muddy_river",
        "wythers:gravelly_river",
        "wythers:lantern_river",
        "wythers:savanna_river"
    );
    
    public static final List<String> riverWarm = ImmutableList.of(
        "terralith:warm_river",
        "regions_unexplored:tropical_river",
        "dreamwoods:tropical_river",
        "wythers:badlands_river",
        "wythers:black_river",
        "wythers:deep_desert_river",
        "wythers:desert_river",
		"wythers:jungle_river",
        "wythers:tropical_forest_river"
    );
    
    public static final List<String> plainsGrassyArid = ImmutableList.of(
        "minecraft:savanna",
        "terralith:savanna_badlands",
        "natures_spirit:arid_savanna",
        "natures_spirit:arid_highlands",
        "natures_spirit:wooded_drylands",
        "regions_unexplored:steppe",
        "natures_spirit:xeric_plains",
        "dreamwoods:arid_plains",
        "windswept:flowering_savanna",
        "wythers:dry_tropical_grassland",
        "wythers:flooded_savanna",
        "wythers:flowering_pantanal",
        "wythers:jacaranda_savanna"
    );
    
    public static final List<String> plainsGrassySnow = ImmutableList.of(
        "minecraft:snowy_plains",
        "terralith:mountain_steppe",
        "natures_spirit:tundra",
        "dreamwoods:fall_snowy_grove",
        "wythers:tundra"
    );
    
    public static final List<String> plainsGrassyTemperate = ImmutableList.of(
        "minecraft:plains",
        "minecraft:sunflower_plains",
        "terralith:highlands",
        "natures_spirit:prairie",
        "regions_unexplored:grassland",
        "regions_unexplored:clover_plains",
        "natures_spirit:heather_fields",
        "natures_spirit:alpine_clearings",
        "dreamwoods:lush_grassland",
        "dreamwoods:lush_meadow",
        "dreamwoods:poplar_fields",
        "dreamwoods:prairie",
        "wythers:cool_plains",
        "wythers:cold_plains",
        "wythers:harvest_fields",
        "wythers:highland_plains"
    );
    
    public static final List<String> plainsGrassyTropical = ImmutableList.of(
        "minecraft:sparse_jungle",
		"wythers:subtropical_grassland",
        "wythers:tropical_grassland"
    );
    
    public static final List<String> plainsShrubbyArid = ImmutableList.of(
        "minecraft:savanna_plateau",
        "terralith:hot_shrubland",
        "regions_unexplored:dry_bushland",
        "regions_unexplored:baobab_savanna",
        "regions_unexplored:outback",
        "dreamwoods:savanna_grove",
        "natures_spirit:shrubland",
        "natures_spirit:shrubby_highlands",
        "natures_spirit:chaparral",
        "wythers:chaparral",
        "wythers:dry_tropical_forest",
        "wythers:dry_savanna",
        "wythers:outback",
        "wythers:savanna_badlands",
		"wythers:savanna_basaltic_incursions",
        "wythers:scrub_forest",
		"wythers:scrubland",
		"wythers:wooded_savanna"
    );
    
    public static final List<String> plainsShrubbySnow = ImmutableList.of(
        "minecraft:grove",
        "terralith:cold_shrubland",
        "terralith:alpine_grove",
        "regions_unexplored:frozen_tundra",
        "windswept:snowy_chestnut_forest",
        "dreamwoods:tundra",
        "windswept:tundra",
        "wythers:crimson_tundra",
        "wythers:snowy_tundra"
    );
    
    public static final List<String> plainsShrubbyTemperate = ImmutableList.of(
        "terralith:steppe",
        "terralith:shrubland",
        "terralith:rocky_shrubland",
        "regions_unexplored:shrubland",
        "natures_spirit:shrubland",
        "regions_unexplored:highland_fields",
        "regions_unexplored:chalk_cliffs",
        "dreamwoods:grassy_shore",
        "dreamwoods:highlands",
        "dreamwoods:moor",
        "dreamwoods:boskage",
        "wythers:highlands",
		"wythers:huangshan_highlands"
    );
    
    public static final List<String> plainsShrubbyTropical = ImmutableList.of(
        "terralith:brushland",
        "dreamwoods:jungle_bushland",
        "wythers:humid_tropical_grassland",
        "wythers:sandy_jungle"
    );
    
    public static final List<String> specialAutumnalFields = ImmutableList.of(
        "minecraft:old_growth_birch_forest",
        "regions_unexplored:barley_fields",
        "regions_unexplored:pumpkin_fields",
        "natures_spirit:golden_wilds",
        "natures_spirit:marigold_meadows",
        "dreamwoods:fall_fruitfull_grove",
        "wythers:autumnal_plains",
        "wythers:autumnal_forest_edge"
    );
    
    public static final List<String> specialAutumnalForest = ImmutableList.of(
        "regions_unexplored:autumnal_maple_forest",
        "natures_spirit:maple_woodlands",
        "regions_unexplored:silver_birch_forest",
        "natures_spirit:aspen_forest",
        "dreamwoods:autumnal_wood",
        "dreamwoods:ruby_maple_forest",
        "dreamwoods:seasonal_grove",
        "wythers:ancient_copper_beech_forest",
		"wythers:ancient_emerald_beech_forest",
		"wythers:ancient_golden_beech_forest",
        "wythers:autumnal_birch_forest",
		"wythers:autumnal_flower_forest",
		"wythers:autumnal_forest"
    );
    
    public static final List<String> specialCraggySnow = ImmutableList.of(
        "minecraft:frozen_peaks",
        "dreamwoods:autumnal_hills",
        "terralith:glacial_chasm",
        "wythers:cold_stony_shore",
        "wythers:cold_plains",
		"wythers:cold_stony_shore",
        "wythers:snowy_canyon"
    );
    
    public static final List<String> specialCraggyTemperate = ImmutableList.of(
        "minecraft:windswept_hills",
        "minecraft:windswept_forest",
        "terralith:basalt_cliffs",
        "terralith:granite_cliffs",
        "terralith:white_cliffs",
        "terralith:yosemite_cliffs",
        "natures_spirit:alpine_highlands",
        "natures_spirit:windswept_sugi_forest",
        "natures_spirit:white_cliffs",
        "dreamwoods:moor",
        "dreamwoods:highlands",
        "dreamwoods:wooded_slopes",
        "dreamwoods:spruce_slopes",
        "dreamwoods:calcite_plateau",
        "dreamwoods:thermal_springs",
        "dreamwoods:boskage",
        "wythers:andesite_crags",
		"wythers:aspen_crags",
        "wythers:autumnal_crags",
        "wythers:calcite_coast",
        "wythers:granite_canyon",
        "wythers:old_growth_taiga_crags",
		"wythers:temperate_rainforest_crags",
        "wythers:taiga_crags",
        "wythers:stony_canyon",
        "wythers:thermal_taiga_crags"
    );
    
    public static final List<String> specialCraggyWarm = ImmutableList.of(
        "minecraft:windswept_savanna",
        "minecraft:savanna_plateau",
        "terralith:fractured_savanna",
        "terralith:amethyst_canyon",
        "wythers:jungle_canyon",
        "wythers:tropical_forest_canyon",
		"wythers:warm_stony_shore",
		"wythers:windswept_jungle",
        "wythers:tsingy_forest"
    );
    
    public static final List<String> specialFloweryFields = ImmutableList.of(
        "minecraft:meadow",
        "minecraft:sunflower_plains",
        "terralith:blooming_plateau",
        "regions_unexplored:flower_fields",
        "regions_unexplored:rocky_meadow",
        "natures_spirit:carnation_fields",
        "natures_spirit:lavender_fields",
        "regions_unexplored:poppy_fields",
        "natures_spirit:blooming_highlands",
        "natures_spirit:flowering_shrubland",
        "natures_spirit:marigold_meadows",
        "dreamwoods:alpine_fields",
        "dreamwoods:lush_grassland",
        "dreamwoods:lush_meadow",
        "windswept:lavender_fields",
        "windswept:lavender_hills",
        "wythers:lapacho_plains",
        "wythers:spring_flower_fields"
    );
    
    public static final List<String> specialFloweryForest = ImmutableList.of(
        "minecraft:flower_forest",
        "terralith:lavender_forest",
        "terralith:lavender_valley",
        "terralith:moonlight_valley",
        "natures_spirit:wisteria_forest",
        "natures_spirit:floral_ridges",
        "dreamwoods:blooming_grove",
        "wythers:autumnal_flower_forest",
        "wythers:spring_flower_forest"
    );
    
    public static final List<String> specialMediterraneanInviting = ImmutableList.of(
        "natures_spirit:cypress_fields",
        "natures_spirit:lavender_fields",
        "windswept:flowering_savanna",
        "wythers:mediterranean_island"
    );
    
    public static final List<String> specialMediterraneanUninviting = ImmutableList.of(
        "natures_spirit:shrubby_highlands",
        "natures_spirit:woody_highlands",
        "natures_spirit:xeric_plains",
        "wythers:mediterranean_island_thermal_springs"
    );
    
    public static final List<String> specialOrientalInviting = ImmutableList.of(
        "minecraft:cherry_grove",
        "terralith:sakura_valley",
        "terralith:sakura_grove",
        "regions_unexplored:rocky_meadow",
        "wythers:sakura_forest"
    );
    
    public static final List<String> specialOrientalUninviting = ImmutableList.of(
        "minecraft:bamboo_jungle",
        "regions_unexplored:bamboo_forest",
        "regions_unexplored:magnolia_woodland",
        "natures_spirit:bamboo_wetlands",
        "aromatic:cinnamon_rainforest",
        "natures_spirit:sugi_forest",
        "natures_spirit:blooming_sugi_forest",
        "natures_spirit:windswept_sugi_forest",
        "wythers:bamboo_jungle_swamp",
		"wythers:bamboo_swamp",
        "wythers:jade_highlands"
    );
    
    public static final List<String> specialIcy = ImmutableList.of(
        "minecraft:ice_spikes",
        "terralith:glacial_chasm",
        "terralith:snowy_badlands",
        "regions_unexplored:spires",
        "dreamwoods:ice_abyss",
        "dreamwoods:ice_grove",
        "wythers:glacial_cliffs",
        "wythers:icy_volcano",
        "wythers:icy_crags"
    );
    
    public static final List<String> specialMagical = ImmutableList.of(
        "minecraft:dark_forest",
        "ars_nouveau:archwood_forest",
        "terralith:cloud_forest",
        "terralith:haze_mountain",
        "terralith:moonlight_valley",
        "terralith:moonlight_grove",
        "terralith:mirage_isles",
        "regions_unexplored:blackwood_taiga",
        "regions_unexplored:fungal_fen",
        "regions_unexplored:flower_fields",
        "regions_unexplored:mauve_hills",
        "natures_spirit:wisteria_forest",
        "dreamwoods:elder_woods",
        "dreamwoods:sacred_giants_wood",
        "dreamwoods:thaumic_grove",
        "dreamwoods:twilight_wetlands"
    );
    
    public static final List<String> specialRuined = ImmutableList.of(
        "regions_unexplored:ashen_woodland"
    );
    
    public static final List<String> specialRocky = ImmutableList.of(
        "minecraft:stony_shore",
        "minecraft:windswept_gravelly_hills",
        "terralith:windswept_spires",
        "terralith:stony_spires",
        "terralith:gravel_desert",
        "wythers:stony_canyon"
    );
    
    public static final List<String> specialShroomy = ImmutableList.of(
        "minecraft:mushroom_fields",
        "terralith:warped_mesa",
        "terralith:orchid_swamp",
        "regions_unexplored:fungal_fen",
        "regions_unexplored:blackwood_taiga",
        "dreamwoods:thaumic_grove",
        "dreamwoods:twilight_wetlands",
        "wythers:mushroom_island"
    );
    
    public static final List<String> specialSpooky = ImmutableList.of(
        "minecraft:pale_garden",
        "nyctophobia:ancient_dead_coral_reef",
        "nyctophobia:deep_dark_forest",
        "nyctophobia:eroded_haunted_forest",
        "nyctophobia:haunted_forest",
        "nyctophobia:haunted_lakes",
        "sleepy_hollows:sleepy_hollows",
        "dreamwoods:grim_woods",
        "dreamwoods:twilight_wetlands",
        "wythers:tangled_forest",
        "wythers:forbidden_forest",
        "wythers:phantasmal_forest",
		"wythers:phantasmal_swamp",
		"wythers:wistman_woods"
    );
    
    public static final List<String> specialSwampySnow = ImmutableList.of(
        "terralith:ice_marsh",
        "terralith:orchid_swamp",
        "dreamwoods:northern_swamp",
        "wythers:snowy_bog",
		"wythers:snowy_fen"
    );
    
    public static final List<String> specialSwampyTemperate = ImmutableList.of(
        "minecraft:swamp",
        "hexerei:willow_swamp",
        "regions_unexplored:fungal_fen",
        "regions_unexplored:fen",
        "regions_unexplored:marsh",
        "natures_spirit:marsh",
        "dreamwoods:weeping_grove",
        "dreamwoods:old_growth_swampland",
        "dreamwoods:morass",
        "wythers:ancient_mossy_swamp",
		"wythers:ancient_oak_swamp",
        "wythers:autumnal_swamp",
        "wythers:berry_bog",
		"wythers:billabong",
		"wythers:birch_swamp",
        "wythers:fen",
        "wythers:flooded_temperate_rainforest",
        "wythers:marsh",
		"wythers:waterlily_swamp",
		"wythers:old_growth_taiga_swamp"
    );
    
    public static final List<String> specialSwampyWarm = ImmutableList.of(
        "minecraft:mangrove_swamp",
        "wildbackport:mangrove_swamp",
        "regions_unexplored:bayou",
        "regions_unexplored:old_growth_bayou",
        "natures_spirit:tropical_basin",
        "dreamwoods:flooded_rainforest",
        "dreamwoods:bayou",
        "wythers:bayou",
        "wythers:bamboo_jungle_swamp",
		"wythers:bamboo_swamp",
        "wythers:coastal_mangroves",
        "wythers:dripleaf_swamp",
        "wythers:flooded_jungle",
		"wythers:flooded_rainforest",
        "wythers:pantanal"
    );
}
