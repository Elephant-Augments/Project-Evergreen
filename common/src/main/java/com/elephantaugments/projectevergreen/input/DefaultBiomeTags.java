...
public class DefaultBiomeTags {
    ...
    public static ArrayListMultimap<String, String> parseDefaultBiomeTags() {
		ArrayListMultimap<String, String> biomesByTag = ArrayListMultimap.create();
        desertDunesAlive.forEach(s -> biomesByTag.put(DESERT_DUNES_ALIVE, s));
        desertDunesBarren.forEach(s -> biomesByTag.put(DESERT_DUNES_BARREN, s));
        desertRedAlive.forEach(s -> biomesByTag.put(DESERT_RED_ALIVE, s));
        desertRedBarren.forEach(s -> biomesByTag.put(DESERT_RED_BARREN, s));
        forestDenseConiferousSnow.forEach(s -> biomesByTag.put(FOREST_DENSE_CONIFEROUS_SNOW, s));
        forestDenseConiferous.forEach(s -> biomesByTag.put(FOREST_DENSE_CONIFEROUS, s));
        forestDenseDeciduous.forEach(s -> biomesByTag.put(FOREST_DENSE_DECIDUOUS, s));
        forestDenseTropical.forEach(s -> biomesByTag.put(FOREST_DENSE_TROPICAL, s));
        forestSparseConiferousSnow.forEach(s -> biomesByTag.put(FOREST_SPARSE_CONIFEROUS_SNOW, s));
        forestSparseConiferous.forEach(s -> biomesByTag.put(FOREST_SPARSE_CONIFEROUS, s));
        forestSparseDeciduous.forEach(s -> biomesByTag.put(FOREST_SPARSE_DECIDUOUS, s));
        forestSparseTropical.forEach(s -> biomesByTag.put(FOREST_SPARSE_TROPICAL, s));
        mountainsAlive.forEach(s -> biomesByTag.put(MOUNTAINS_ALIVE, s));
        mountainsBarren.forEach(s -> biomesByTag.put(MOUNTAINS_BARREN, s));
        mountainsCold.forEach(s -> biomesByTag.put(MOUNTAINS_COLD, s));
        mountainsHot.forEach(s -> biomesByTag.put(MOUNTAINS_HOT, s));
        oceanDeepFrozen.forEach(s -> biomesByTag.put(OCEAN_DEEP_FROZEN, s));
        oceanDeepTemperate.forEach(s -> biomesByTag.put(OCEAN_DEEP_TEMPERATE, s));
        oceanDeepWarm.forEach(s -> biomesByTag.put(OCEAN_DEEP_WARM, s));
        oceanShallowFrozen.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_FROZEN, s));
        oceanShallowTemperate.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_TEMPERATE, s));
        oceanShallowWarm.forEach(s -> biomesByTag.put(OCEAN_SHALLOW_WARM, s));
        coastalFrozen.forEach(s -> biomesByTag.put(COASTAL_FROZEN, s));
        coastalTemperate.forEach(s -> biomesByTag.put(COASTAL_TEMPERATE, s));
        coastalWarm.forEach(s -> biomesByTag.put(COASTAL_WARM, s));
        coastalAlive.forEach(s -> biomesByTag.put(COASTAL_ALIVE, s));
        coastalBarren.forEach(s -> biomesByTag.put(COASTAL_BARREN, s));
        riverFrozen.forEach(s -> biomesByTag.put(RIVER_FROZEN, s));
        riverTemperate.forEach(s -> biomesByTag.put(RIVER_TEMPERATE, s));
        riverWarm.forEach(s -> biomesByTag.put(RIVER_WARM, s));
        plainsGrassyArid.forEach(s -> biomesByTag.put(PLAINS_GRASSY_ARID, s));
        plainsGrassySnow.forEach(s -> biomesByTag.put(PLAINS_GRASSY_SNOW, s));
        plainsGrassyTemperate.forEach(s -> biomesByTag.put(PLAINS_GRASSY_TEMPERATE, s));
        plainsGrassyTropical.forEach(s -> biomesByTag.put(PLAINS_GRASSY_TROPICAL, s));
        plainsShrubbyArid.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_ARID, s));
        plainsShrubbySnow.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_SNOW, s));
        plainsShrubbyTemperate.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_TEMPERATE, s));
        plainsShrubbyTropical.forEach(s -> biomesByTag.put(PLAINS_SHRUBBY_TROPICAL, s));
        specialAutumnalFields.forEach(s -> biomesByTag.put(SPECIAL_AUTUMNAL_FIELDS, s));
        specialAutumnalForest.forEach(s -> biomesByTag.put(SPECIAL_AUTUMNAL_FOREST, s));
        specialCraggySnow.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_SNOW, s));
        specialCraggyTemperate.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_TEMPERATE, s));
        specialCraggyWarm.forEach(s -> biomesByTag.put(SPECIAL_CRAGGY_WARM, s));
        specialFloweryFields.forEach(s -> biomesByTag.put(SPECIAL_FLOWERY_FIELDS, s));
        specialFloweryForest.forEach(s -> biomesByTag.put(SPECIAL_FLOWERY_FOREST, s));
        specialMediterraneanInviting.forEach(s -> biomesByTag.put(SPECIAL_MEDITERRANEAN_INVITING, s));
        specialMediterraneanUninviting.forEach(s -> biomesByTag.put(SPECIAL_MEDITERRANEAN_UNINVITING, s));
        specialOrientalInviting.forEach(s -> biomesByTag.put(SPECIAL_ORIENTAL_INVITING, s));
        specialOrientalUninviting.forEach(s -> biomesByTag.put(SPECIAL_ORIENTAL_UNINVITING, s));
        specialIcy.forEach(s -> biomesByTag.put(SPECIAL_ICY, s));
        specialMagical.forEach(s -> biomesByTag.put(SPECIAL_MAGICAL, s));
        specialRuined.forEach(s -> biomesByTag.put(SPECIAL_RUINED, s));
        specialShroomy.forEach(s -> biomesByTag.put(SPECIAL_SHROOMY, s));
        specialSpooky.forEach(s -> biomesByTag.put(SPECIAL_SPOOKY, s));
        specialSwampySnow.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_SNOW, s));
        specialSwampyTemperate.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_TEMPERATE, s));
        specialSwampyWarm.forEach(s -> biomesByTag.put(SPECIAL_SWAMPY_WARM, s));
        return biomesByTag;
    }
